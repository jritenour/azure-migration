...
@subscription_id=@provider.subscription
  ...
  vns = Azure::Armrest::Network::VirtualNetworkService.new(net) 
  netnames= vns.list_all.map {|x| [x["name"],x["name"]]}

  $evm.log(:info, "Inspecting Network Names: #{netnames.inspect}")  


    # set the values  
  dialog_field['values'] = netnames.to_a
